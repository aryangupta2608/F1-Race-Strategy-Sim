...
# Default pit loss time (seconds) used when circuit not in the lookup table.
# Based on real 2024 F1 pit stop summary data from formula1.com.
DEFAULT_PIT_LOSS = 22.0

# Circuit-specific pit lane time loss in seconds.
# These are the total delta (entry + stationary + exit) derived from
# official F1 pit stop summary data — not just the stationary time.
# Stationary time (~2.2s) is included; what varies is the pit lane length
# and speed limit (most circuits 80 km/h, Monaco 60 km/h).
PIT_LOSS_BY_CIRCUIT = {
    # ── Short pit lanes (~19–22s) ─────────────────────────────
    "Bahrain Grand Prix":           22.0,
    "Saudi Arabian Grand Prix":     22.5,
    "Monaco Grand Prix":            20.0,  # short lane but 60 km/h limit
    "Austrian Grand Prix":          20.0,
    "Dutch Grand Prix":             21.0,
    "Las Vegas Grand Prix":         21.0,
    "Qatar Grand Prix":             21.5,

    # ── Medium pit lanes (~22–25s) ────────────────────────────
    "Australian Grand Prix":        23.0,
    "Chinese Grand Prix":           23.0,
    "Japanese Grand Prix":          24.0,
    "Emilia Romagna Grand Prix":    23.5,
    "Spanish Grand Prix":           23.5,
    "Hungarian Grand Prix":         23.0,
    "Belgian Grand Prix":           23.5,
    "Italian Grand Prix":           23.0,
    "United States Grand Prix":     23.0,
    "Mexico City Grand Prix":       23.5,
    "São Paulo Grand Prix":         23.0,
    "Abu Dhabi Grand Prix":         23.5,
    "Miami Grand Prix":             22.5,
    "Azerbaijan Grand Prix":        22.0,
    "Canadian Grand Prix":          23.0,

    # ── Long pit lanes (~26–30s) ──────────────────────────────
    "British Grand Prix":           28.5,  # Silverstone is notably long
    "Singapore Grand Prix":         29.5,  # longest on the calendar
}


def get_pit_loss_time(event_name: str) -> float:
    """
    Returns the pit lane time loss in seconds for a given Grand Prix.
    Tries exact match first, then partial match on the event name.
    Falls back to DEFAULT_PIT_LOSS if not found.

    Parameters:
        event_name — FastF1 event name e.g. "Bahrain Grand Prix"
    """
    # Exact match
    if event_name in PIT_LOSS_BY_CIRCUIT:
        return PIT_LOSS_BY_CIRCUIT[event_name]

    # Partial match — handles slight name variations
    for key in PIT_LOSS_BY_CIRCUIT:
        if key.lower() in event_name.lower() or event_name.lower() in key.lower():
            return PIT_LOSS_BY_CIRCUIT[key]

    # Fallback
    print(f"  ⚠️  No circuit-specific pit loss for '{event_name}', using default {DEFAULT_PIT_LOSS}s")
...
def simulate_strategy(model: dict, total_laps: int, strategy: list, pit_loss: float = DEFAULT_PIT_LOSS) -> dict:
    """
    ...
    for stint_idx, (compound, stint_length) in enumerate(strategy):
        ...
        # Record pit stop lap and insert pit loss as a visible spike in lap times
        # This makes the chart show a jump at pit stop laps rather than a drop,
        # which is what actually happens: the pit lap itself is very slow (~22s added)
        if stint_idx < len(strategy) - 1:
            pit_lap_number = current_lap - 1
            pit_laps.append(pit_lap_number)
            # Add the pit loss time onto the last lap of this stint in the lap list
            # so the chart shows a spike at the pit stop lap
            if lap_times:
                lap_times[-1] = round(lap_times[-1] + pit_loss, 3)

    # Add pit loss time for each stop
    n_stops    = len(strategy) - 1
    pit_time   = n_stops * pit_loss
...
def generate_strategies(model: dict, total_laps: int, max_stops: int = 2, pit_loss: float = DEFAULT_PIT_LOSS) -> list:
    """
    ...
    if max_stops >= 1:
        ...
        for c1, c2 in compound_pairs:
            ...
            for pit_lap in range(8, total_laps - 8, 3):
                ...
                result   = simulate_strategy(model, total_laps, strategy, pit_loss)
                if result['valid']:
...
def find_optimal_strategy(model: dict, total_laps: int, max_stops: int = 2, event_name: str = "") -> dict:
    """
    ...
    results = generate_strategies(model, total_laps, max_stops, pit_loss)
...
if __name__ == "__main__":
    ...
    print("   Ready for Milestone 4: Streamlit app (app.py)")# Time lost in the pit lane per stop (seconds).
